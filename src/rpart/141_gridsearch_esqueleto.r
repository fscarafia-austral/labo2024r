...
PARAM$semillas <- c(101957, 190657, 123007, 411991, 878851) #semillas franco
...
corrida = 0
div_minsplit = 1.5^c(2:5)

# minbucket debe ser menos de la mitad del minsplit
# consejo: minbucket = minsplit/(1.5^n) 
# itero por los loops anidados para cada hiperparametro
inicio <- Sys.time()
for (vmax_depth in c(4, 6, 8, 10, 12, 14)) {
  for (vmin_split in c(1000, 800, 600, 400, 200, 100, 50, 20, 10)) {
    
    vmin_bucket_values <- vmin_split/div_minsplit
    
    for (vcp in seq(-0.08,0.02,0.02)){
     for (vmin_bucket_i in vmin_bucket_values){
       vmin_bucket = as.integer(vmin_bucket_i)
       corrida = corrida + 1
       ...
       tb_grid_search <- rbindlist( 
         list( tb_grid_search, 
               list( vcp, vmin_bucket, vmax_depth, vmin_split, ganancia_promedio) ) )
       
       print(paste0("Corrida: ", corrida,
                    " / CP: ", vcp,
